...
namespace alexis
{
	enum PBSObjectParameters
	{
		...
		ModelCB,
		BaseColor,
		MetalRoughness, //float4
		NumPBSObjectParameters
	};

	PBSSimple::PBSSimple(const PBSMaterialParams& params) :
		m_baseColor(params.BaseColor),
		m_metallRoughness(XMVectorSet(params.Metallic, params.Roughness, 0.0f, 0.0f))
	{
		// TODO: move shader loading to resource manager

#if defined(_DEBUG)
		ThrowIfFailed(D3DReadFileToBlob(L"Resources/Shaders/PBS_object_simple_vs_d.cso", &m_vertexShader));
		ThrowIfFailed(D3DReadFileToBlob(L"Resources/Shaders/PBS_object_simple_ps_d.cso", &m_pixelShader));
#else
		ThrowIfFailed(D3DReadFileToBlob(L"Resources/Shaders/PBS_object_simple_vs.cso", &m_vertexShader));
		ThrowIfFailed(D3DReadFileToBlob(L"Resources/Shaders/PBS_object_simple_ps.cso", &m_pixelShader));
#endif
		...
		rootParameters[PBSObjectParameters::BaseColor].InitAsDescriptorTable(1, &descriptorRange, D3D12_SHADER_VISIBILITY_PIXEL);
		rootParameters[PBSObjectParameters::MetalRoughness].InitAsConstantBufferView(0, 1, D3D12_ROOT_DESCRIPTOR_FLAG_NONE, D3D12_SHADER_VISIBILITY_PIXEL);
