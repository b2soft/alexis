...
cbuffer GenerateMipsCB : register(b0)
{
	...
	uint SrcDimension; // Width and height of the src texture
...
{
	return x < 0.4045f ? x / 12.92 : pow((x + 0.055) / 1.055, 2.4);
}
...
void main(ComputeShaderInput IN)
{
	...
	case WIDTH_ODD_HEIGHT_EVEN:
	{
		...
		Src1 = 0.5 * (SrcMip.SampleLevel(LinearClampSampler, UV1, SrcMipLevel) +
			SrcMip.SampleLevel(LinearClampSampler, UV1 + Offset, SrcMipLevel));
	}
	break;
	...
	case WIDTH_HEIGHT_ODD:
	{
		...
		Src1 += SrcMip.SampleLevel(LinearClampSampler, UV1 + float2(Offset.x, 0.0), SrcMipLevel);
		Src1 += SrcMip.SampleLevel(LinearClampSampler, UV1 + float2(0.0, Offset.y), SrcMipLevel);
		Src1 += SrcMip.SampleLevel(LinearClampSampler, UV1 + float2(Offset.x, Offset.y), SrcMipLevel);
		Src1 *= 0.25;
	}
	break;
	}
	...
	// This guarantees all LDS writes are complete and that all threads have executed all instructions so far (and therefore have issued their LDS write instructions)
